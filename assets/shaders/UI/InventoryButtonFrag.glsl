...
void main() {
	...
	if (col.a <= 0.1) {
		...
		col.xyz = mix(vec3(0.05, 0.05, 0.05),vec3(0.15, 0.1, 0.15),((noise((texCoords + (vec2(cos(Time * 20 * pi), sin(Time * 20 * pi)) * 1)) * 30) + 1)/2.0));
		col.xyz = mix(col.xyz, col.xyz + vec3(0.1, 0.02, 0.1),((noise((texCoords + (vec2(cos(Time * 100 * pi), sin(Time * 100 * pi)) * 0.1)) * 60) + 1)/2.0));
