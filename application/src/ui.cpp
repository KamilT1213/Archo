...
void UI::init(const glm::ivec2& size)
{
	...
	//m_UIScene->m_entities.reserve(3); // Quads, circles, rounded quads
...

	{
		...
		m_quad = m_UIScene->m_entities.create();

		// Add a Render component adn keep a reference to it in this scope
		auto& renderComp = m_UIScene->m_entities.emplace<Render>(m_quad);
		// Set geometry and material
		renderComp.geometry = quadsVAO;
		renderComp.material = quadsMaterial;

		renderComp.material->setValue("u_textureSlots[0]", slots.data());
		renderComp.geometry->overrideDrawCount(0);

		// Add a transform component
		m_UIScene->m_entities.emplace<Transform>(m_quad);
	}
...

	{
		...
		// Add a Render component adn keep a reference to it in this scope
		auto& renderComp = m_UIScene->m_entities.emplace<Render>(m_circles);
		// Set geometry and material
		...
		// Add a transform component
		//m_circleIdx = m_UIScene->m_entities.group<Transform>().size();

		m_UIScene->m_entities.emplace<Transform>(m_circles);
	}
...
void UI::begin()
{
	...
	auto& quadRender = m_UIScene->m_entities.get<Render>(m_quad);
	quadRender.geometry->overrideDrawCount(0);

	// Clear circle data

	m_currentCircleCount = 0;
	auto& circleRender = m_UIScene->m_entities.get<Render>(m_circles);
	circleRender.geometry->overrideDrawCount(0);

	//m_UIScene->m_entities.at(m_quadIdx).geometry->overrideDrawCount(0);
	
	//m_UIScene->m_entities.at(m_circleIdx).geometry->overrideDrawCount(0);
}
...
void UI::end()
{
	...
	auto& quadRender = m_UIScene->m_entities.get<Render>(m_quad);
	quadRender.geometry->overrideDrawCount(m_currentQuadCount * 6);

	// Send circle vertices to the SSBO
	m_circlesSSBO->edit(0, sizeof(CircleVertex) * m_currentCircleCount * 4, m_circleVertices.data());

	auto& circleRender = m_UIScene->m_entities.get<Render>(m_circles);
	circleRender.geometry->overrideDrawCount(m_currentCircleCount * 6);

	//// Set quad draw count
	//m_UIScene->m_entities.at(m_quadIdx).geometry->overrideDrawCount(m_currentQuadCount * 6);

	//
	//
	//// Set circle draw count
	//m_UIScene->m_entities.at(m_circleIdx).geometry->overrideDrawCount(m_currentCircleCount * 6);
}
