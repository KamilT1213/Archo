...
Archo::Archo(GLFWWindowImpl& win) : Layer(win)
{
	...
	for (int i = 0; i < Relics; i++) {
		...
		Transform& transfromComp = m_RelicScene->m_entities.emplace<Transform>(relic);
		...
		transfromComp.scale = glm::vec3(Randomiser::uniformFloatBetween(100.0f, 150.0f) * ((0.5f * ((7.0f - (1 + rarity)) / 7.0f)) + 0.5f));

		transfromComp.translation = glm::vec3(Randomiser::uniformFloatBetween(transfromComp.scale.x, 4096.0f - transfromComp.scale.x), Randomiser::uniformFloatBetween(transfromComp.scale.y, 4096.0f - transfromComp.scale.y), (1.0f - (rarity / 6.0f)) - 0.5f);


		renderComp.material = RelicMaterial;

		transfromComp.recalc();
	}
