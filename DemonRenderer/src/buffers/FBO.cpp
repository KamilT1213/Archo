...
void FBO::onResize(WindowResizeEvent& e,int scale)
{
	


	if (m_ID > 0) {
		glm::ivec2 size = e.getSize() / scale;

		m_sampledTargets.clear();
		m_nonSampledTargets.clear();
		m_colAttachments.clear();
	...
	//spdlog::warn("New sizes: W:{} H:{} At new location: {}", e.getWidth(), e.getHeight(),m_ID);
	// TO DO
